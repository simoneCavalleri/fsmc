...
_fsmc() {
    ...
    options=(
        ...
        '(-o --output)'{-o,--output}'[Output generated code or diagram file]:file:_files'
        '(-t --target)'{-t,--target}'[Target backend]:target:(cpp c rust zig ts python)'
        '(-n --name)'{-n,--name}'[FSM class name]:name:'
        '--namespace[Namespace/Package name]:namespace:'
        '--package[Namespace/Package name]:package:'
        ...
        '--format[Override model format]:format:(auto cameo scxml json dot sysml2 plantuml mermaid)'
        '(-e --export)'{-e,--export}'[Export diagram format]:export:(mermaid plantuml sysml2 json)'
        '--verify[Run formal model checker and exit]'
        '--check[Run formal model checker and exit]'
