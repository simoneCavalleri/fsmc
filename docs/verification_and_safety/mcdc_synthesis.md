...
## 3. Harness Generation: C++ API and CLI Workflow

MC/DC test synthesis is powered by the `McdcHarnessGenerator` engine (`include/fsm/backend/verification/mcdc_harness_generator.hpp`).
...
// Generate standalone GoogleTest C++ test harness from model AST
std::string gtest_suite = fsm::backend::verification::McdcHarnessGenerator::generate_gtest_harness(model);
```

### CLI Usage

The `--emit-test-harness` option exposes this generator directly from the `fsmc` compiler driver:
