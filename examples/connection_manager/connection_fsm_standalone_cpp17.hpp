...
template <typename T>
struct has_on_enter<T, std::void_t<decltype(&T::on_enter)>> : std::true_type {};

template <typename T, typename = void>
struct has_on_exit : std::false_type {};
template <typename T>
struct has_on_exit<T, std::void_t<decltype(&T::on_exit)>> : std::true_type {};
}  // namespace detail
