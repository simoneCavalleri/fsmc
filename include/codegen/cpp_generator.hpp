...
struct GeneratorOptions {
  ...
  bool include_stubs =
      true;  // Generate default functor definitions for guards and actions
  bool thread_safe = true;  // Generate thread_safe_fsm wrapper
};

class CppGenerator {
 public:
  static std::string generate_header(const FsmModel& model,
                                     const GeneratorOptions& options = {}) {
    std::ostringstream out;

    std::string std_str =
        (options.cpp_standard == CppStandard::Cpp20) ? "C++20" : "C++17";

    out << "// "
           "==================================================================="
           "=========\n";
    out << "// Auto-generated by fsm-gen (" << std_str
    ...
    if (!model.context_type.empty() && model.context_type != "no_context" &&
        model.context_type != "fsm::no_context" &&
        model.context_type != "void") {
    ...
    std::string ctx_type =
        (model.context_type.empty() || model.context_type == "no_context" ||
         model.context_type == "void")
            ? "fsm::no_context"
            : model.context_type;
    ...
    out << "using " << model.name << " = fsm::fsm<" << table_type_name << ", "
        << ctx_type << ", " << init_state << ">;\n";

    if (options.thread_safe) {
      out << "using ThreadSafe" << model.name << " = fsm::thread_safe_fsm<"
          << table_type_name << ", " << ctx_type << ", " << init_state
          << ">;\n";
    }
 ...
 private:
  static void generate_cpp20_standalone_runtime(std::ostream& out,
    ...
    out << "template <typename State, typename Evt, typename GuardType = no_guard, typename ActionType = no_action>\n";
  ...
  static void generate_cpp17_standalone_runtime(std::ostream& out,
                                                const GeneratorOptions& opts) {
    out << "#include <variant>\n";
    out << "#include <tuple>\n";
    out << "#include <string_view>\n";
