...
class Cpp17StandaloneRuntime {
  public:
    static void emit(std::ostream& out, const GeneratorOptions& opts) {
        ...
        out << "struct no_guard {\n";
        out << "    template <typename... Args> constexpr bool operator()(const Args&...) const noexcept { return "
               "true; }\n";
        out << "};\n\n";

        out << "struct no_action {\n";
        out << "    template <typename... Args> constexpr void operator()(Args&...) const noexcept {}\n";
        out << "};\n\n";
        ...
        out << "namespace detail {\n";
        out << "template <typename T, typename = void> struct has_name : std::false_type {};\n";
        out << "template <typename T> struct has_name<T, std::void_t<decltype(T::name)>> : std::true_type {};\n\n";
        out << "template <typename T, typename = void> struct has_parent : std::false_type {};\n";
        out << "template <typename T> struct has_parent<T, std::void_t<decltype(T::parent)>> : std::true_type {};\n\n";
        out << "template <typename T, typename = void> struct has_on_enter : std::false_type {};\n";
        out << "template <typename T> struct has_on_enter<T, std::void_t<decltype(&T::on_enter)>> : std::true_type "
               "{};\n\n";
        out << "template <typename T, typename = void> struct has_on_exit : std::false_type {};\n";
        out << "template <typename T> struct has_on_exit<T, std::void_t<decltype(&T::on_exit)>> : std::true_type {};\n";
        out << "} // namespace detail\n\n";
        ...
        out << "    if constexpr (detail::has_name<Event>::value) {\n";
        ...
        out << "template <typename Guard, typename Event, typename State, typename Context>\n";
        out << "constexpr bool invoke_guard(const Guard& g, const Event& evt, const State& s, Context* ctx) {\n";
        out << "    if constexpr (std::is_invocable_v<Guard, const Event&, const State&, Context&>) {\n";
        out << "        return ctx ? g(evt, s, *ctx) : true;\n";
        out << "    } else if constexpr (std::is_invocable_v<Guard, const Event&, const State&>) {\n";
        out << "        return g(evt, s);\n";
        ...
        out << "constexpr void invoke_action(const Action& a, const Event& evt, SrcState& src, DstState& dst, Context* "
               "ctx) {\n";
        out << "    if constexpr (std::is_invocable_v<Action, const Event&, SrcState&, DstState&, Context&>) {\n";
        out << "        if (ctx) a(evt, src, dst, *ctx);\n";
        out << "    } else if constexpr (std::is_invocable_v<Action, const Event&, SrcState&, DstState&>) {\n";
        out << "        a(evt, src, dst);\n";
        out << "    } else if constexpr (std::is_invocable_v<Action, const Event&, DstState&>) {\n";
        out << "        a(evt, dst);\n";
        ...
        out << "        if constexpr (std::is_invocable_v<decltype(&State::on_enter), State&, Context&>) {\n";
        out << "            if (ctx) s.on_enter(*ctx);\n";
        out << "        } else if constexpr (std::is_invocable_v<decltype(&State::on_enter), State&>) {\n";
        out << "            s.on_enter();\n";
        out << "        }\n";
        out << "    }\n";
        out << "}\n\n";
        ...
        out << "        if constexpr (std::is_invocable_v<decltype(&State::on_exit), State&, Context&>) {\n";
        out << "            if (ctx) s.on_exit(*ctx);\n";
        out << "        } else if constexpr (std::is_invocable_v<decltype(&State::on_exit), State&>) {\n";
        out << "            s.on_exit();\n";
        out << "        }\n";
        out << "    }\n";
        out << "}\n\n";
        ...
        out << "    template <typename C = Context, typename = std::enable_if_t<std::is_same_v<C, no_context>>>\n";
        out << "    constexpr fsm() : current_state_(InitialState{}), context_(nullptr) { enter_initial(); }\n";
        ...
        out << "            const auto evt_name = ::fsm::get_event_name(event);\n";
        ...
        out << "        if constexpr (std::is_same_v<SrcState, typename Row::source_state> && std::is_same_v<Event, "
               "typename Row::event_type>) {\n";
        out << "            if (!::fsm::invoke_guard(typename Row::guard_type{}, evt, src, context_, *this)) {\n";
        out << "                return dispatch_result{dispatch_status::guard_rejected};\n";
        out << "            }\n";
        out << "            const auto src_name = ::fsm::get_state_name(src);\n";
        out << "            const auto evt_name = ::fsm::get_event_name(evt);\n";
        ...
        out << "                    }\n";
        out << "                    if (src_p != dst_p) {\n";
        out << "                        record_history(src_p, src_name);\n";
        out << "                    }\n";
        out << "                }\n";
        out << "                ::fsm::invoke_exit_hook(src, context_);\n";
        out << "                typename Row::target_state dst_state{};\n";
        out << "                ::fsm::invoke_action(typename Row::action_type{}, evt, src, dst_state, context_);\n";
        out << "                ::fsm::invoke_enter_hook(dst_state, context_);\n";
        out << "                const auto dst_name = ::fsm::get_state_name(dst_state);\n";
        out << "                if (observer_) observer_(transition_info{src_name, dst_name, evt_name, "
               "dispatch_status::success, transition_kind::external});\n";
        out << "                current_state_.template emplace<typename Row::target_state>(std::move(dst_state));\n";
        out << "                return dispatch_result{dispatch_status::success};\n";
        out << "            }\n";
        out << "        }\n";
        ...
        if (opts.thread_safe) {
            ...
            out << "    /** Single-Consumer Polling: processes one event in FIFO order. */\n";
            ...
            out << "        while (true) {\n";
            out << "            std::vector<event_handler> batch;\n";
            out << "            {\n";
            out << "                std::scoped_lock lock(queue_mutex_);\n";
            out << "                if (queue_.empty()) break;\n";
            out << "                while (!queue_.empty()) {\n";
            out << "                    batch.push_back(std::move(queue_.front()));\n";
            out << "                    queue_.pop();\n";
            out << "                }\n";
            out << "            }\n";
            ...
            out << "            if (worker_.joinable()) worker_.join();\n";
            out << "            running_ = true;\n";
            out << "        }\n";
            ...
            out << "        std::scoped_lock lock(queue_mutex_);\n";
            out << "        std::queue<event_handler> empty_q;\n";
            out << "        std::swap(queue_, empty_q);\n";
            out << "    }\n\n";
