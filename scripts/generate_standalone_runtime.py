...
from the canonical modular runtime headers in include/fsm/runtime/cpp/.
...
def clean_file_content(filepath):
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Match local fsm/runtime include (skip it)
            if re.match(r'^#include\s*"fsm/runtime/', stripped):
                continue
...
def main():
    ...
    runtime_dir = os.path.join(args.root, "include/fsm/runtime/cpp")
    ...
    if args.check:
        if is_out_of_date:
            print("Error: Standalone runtime headers are out of sync with include/fsm/runtime/cpp/.", file=sys.stderr)
            print("Run 'cmake --build build --target generate_standalone_runtime' to update.", file=sys.stderr)
            return 1
        print("Success: Standalone runtime headers are 100% in sync with include/fsm/runtime/cpp/.")
