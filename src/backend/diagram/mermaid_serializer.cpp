...
using frontend::directive::DirectiveParser;
using frontend::directive::GuardExpressionParser;

namespace {

void emit_leaf_state(std::ostream& out, const StateNode& state, const std::string& pad) {
    ...
    bool has_actions = !state.entry_actions.empty() || !state.exit_actions.empty() || !state.deferred_events.empty() ||
...
std::string build_label(const TransitionEdge& trans) {
    std::string label;
    if (trans.priority > 0) {
...
void emit_state(std::ostream& out, const StateNode& state, const FsmIr& model,
...
std::string MermaidSerializer::serialize(const FsmIr& model) {
