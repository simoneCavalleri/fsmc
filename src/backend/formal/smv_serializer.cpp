...
std::string sanitize_smv_ident(std::string_view name) {
    std::string res;
    for (char c : name) {
...
void extract_smv_identifiers(const std::string& expr, const std::set<std::string>& known,
...
std::string to_smv_predicate(std::string_view expr) {
    std::string s(expr);
...
std::string format_ast_for_smv(const PropertyAstNode& node, const std::vector<std::string>& event_names) {
...
std::string to_smv_formula(const FormalProperty& prop, const std::vector<std::string>& event_names) {
...
std::string SmvSerializer::serialize(const FsmIr& model) {
    ...
    for (const auto& var : model.variables) {
        ...
        out << "  " << vname << " : " << var.type.to_smv_type(var.min_value, var.max_value) << ";";
    ...
    for (const auto& port : model.ports) {
        ...
        std::optional<int64_t> min_int =
            port.min_value.has_value() ? std::make_optional(static_cast<int64_t>(*port.min_value)) : std::nullopt;
        std::optional<int64_t> max_int =
            port.max_value.has_value() ? std::make_optional(static_cast<int64_t>(*port.max_value)) : std::nullopt;
        out << "  " << pname << " : " << port.type.to_smv_type(min_int, max_int) << ";";
