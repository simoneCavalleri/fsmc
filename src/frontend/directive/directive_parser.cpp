...
std::string extract_quoted_or_word(const std::string& str, std::size_t start) {
    if (start >= str.size())
        return "";
...
std::vector<std::string> extract_array_items(const std::string& str, std::size_t start) {
...
bool DirectiveParser::is_directive(std::string_view line) {
    std::string trimmed = trim(line);
    return trimmed.find("@fsm:") != std::string::npos;
}

std::string DirectiveParser::extract_directive_body(std::string_view line) {
    std::string trimmed = trim(line);
...
bool DirectiveParser::parse_state_directive(std::string_view body, StateNode& state) {
    std::string str(body);
...
bool DirectiveParser::parse_state_directive(std::string_view body, FsmIr& model,
...
bool DirectiveParser::parse_defer_directive(std::string_view body, StateNode& state) {
    std::string str(body);
    ...
    for (auto& ev : list) {
        if (!ev.empty() &&
            std::find(state.deferred_events.begin(), state.deferred_events.end(), ev) == state.deferred_events.end()) {
...
std::optional<SignalDefinition> DirectiveParser::parse_signal_directive(std::string_view body) {
...
std::optional<FormalProperty> DirectiveParser::parse_property_directive(std::string_view body) {
    std::string str = trim(body);
    if (str.empty())
        return std::nullopt;
    ...
    if (f_pos != std::string::npos) {
        ...
        if (eq_pos != std::string::npos) {
            ...
            if (after_eq.rfind("formula=", 0) == std::string::npos && after_eq.rfind("ltl=", 0) == std::string::npos &&
...
std::optional<PortDefinition> DirectiveParser::parse_port_directive(std::string_view body) {
    std::string str = trim(body);
    if (str.empty())
        return std::nullopt;
...
std::optional<VariableDefinition> DirectiveParser::parse_variable_directive(std::string_view body) {
    std::string str = trim(body);
    if (str.empty())
        return std::nullopt;
    ...
    if (k_pos != std::string::npos) {
        ...
        if (k_str == "Boolean")
            ...
            var.type = DataType::uint32();
        else if (k_str == "Float")
            var.type = DataType::float32();
    }
...
std::optional<EnumDefinition> DirectiveParser::parse_enum_directive(std::string_view body) {
    std::string str = trim(body);
    if (str.empty())
        return std::nullopt;
...
std::optional<StructDefinition> DirectiveParser::parse_struct_directive(std::string_view body) {
    std::string str = trim(body);
    if (str.empty())
        return std::nullopt;
...
std::string DirectiveParser::format_enum_directive(const EnumDefinition& en) {
    std::string out =
        "name=" + en.name + " type=" + (en.underlying_type.empty() ? "uint8_t" : en.underlying_type) + " literals=[";
...
std::string DirectiveParser::format_enum_directive(const TypeDefinition& en) {
    std::string out =
        "name=" + en.name + " type=" + (en.underlying_type.empty() ? "uint8_t" : en.underlying_type) + " literals=[";
    for (size_t i = 0; i < en.literals.size(); ++i) {
        if (i > 0)
...
std::string DirectiveParser::format_struct_directive(const StructDefinition& st) {
    ...
    for (size_t i = 0; i < st.fields.size(); ++i) {
        if (i > 0)
            out += ", ";
        out += st.fields[i].name + ":" + st.fields[i].type.to_canonical_string();
...
std::string DirectiveParser::format_struct_directive(const TypeDefinition& st) {
    std::string out = "name=" + st.name;
    if (st.is_datatype) {
        out += " is_datatype=true";
    }
...
bool DirectiveParser::parse_model_directive(std::string_view body, FsmIr& model) {
...
bool DirectiveParser::parse_trans_directive(std::string_view body, TransitionEdge& trans) {
    std::string str(body);
    ...
    if (a_pos == std::string::npos) {
        a_pos = str.find("transition_action=");
        if (a_pos != std::string::npos) {
            a_pos += 18;
        } else {
            a_pos = str.find("action=");
            if (a_pos != std::string::npos) {
                a_pos += 7;
            }
        }
        if (a_pos != std::string::npos) {
            std::string act = extract_quoted_or_word(str, a_pos);
            trans.transition_action = ActionSignature(act, act);
        }
    } else {
