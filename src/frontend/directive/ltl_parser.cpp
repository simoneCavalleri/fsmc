...
class LtlTokenizerAndParser {
  public:
    explicit LtlTokenizerAndParser(std::string_view src) : src_(src) { tokenize(); }
  ...
  private:
    ...
    PropertyAstNode parse_primary() {
        ...
        if (current().type == TokenType::Atom) {
            ...
            advance();
            while (current().type == TokenType::Atom) {
                atom += " " + current().value;
                advance();
            }
...
std::optional<PropertyAstNode> LtlPropertyParser::parse(std::string_view raw_formula) {
    std::string expr = std::string(trim(raw_formula));
    if (expr.empty()) {
        return std::nullopt;
    }

    LtlTokenizerAndParser parser(expr);
