...
bool ScxmlParser::parse(std::string_view content, FsmIr& model, std::string& error_message) {
    std::string xml_err;
    ...
    // W3C SCXML Normative Execution Model: Single-event Run-To-Completion and Document Order
    model.concurrency.dispatch = EventDispatchSemantics::SingleEventRunToCompletion;
    model.concurrency.orthogonal_conflict = OrthogonalConflictResolution::DocumentOrder;
...
/**
 * @brief Recursively parses SCXML XML child nodes into the canonical FsmIr model.
 *
 * Implements a two-pass recursive descent over SCXML DOM nodes:
 * - Pass 1: Registers all state definitions (<state>, <parallel>, <final>) at the current level,
 *   establishing composite state hierarchies, orthogonal regions, initial sub-states, and final state
 *   completion events (e.g. 'done.state.<id>').
 * - Pass 2: Processes nested elements within each state:
 *   * <datamodel> and <data>: internal variables and typed I/O ports.
 *   * <onentry> and <onexit>: entry/exit action effects, variable assignments, and event emissions.
 *   * Hierarchical child states (<state>, <parallel>, <final>) via recursive descent.
 *   * <history>: shallow ([H]) and deep ([H*]) history pseudostate configurations.
 *   * <defer>: deferred event registration for state-local queuing.
 *   * <invoke>: asynchronous state do-activity registration.
 *   * <transition>: guarded and timed transition edges.
 *
 * @param parent_node Current parent XML node in the SCXML DOM tree.
 * @param model Intermediate representation receiving state machine entities.
 * @param current_parent_state Identifier of the enclosing parent state (empty for root).
 */
void ScxmlParser::parse_scxml_children(const std::shared_ptr<XmlNode>& parent_node, FsmIr& model,
                                       const std::string& current_parent_state) {
    // -------------------------------------------------------------------------
    // Pass 1: State Registration
    // Register all state declarations at this hierarchical level in document order
    // -------------------------------------------------------------------------
    for (const auto& child : parent_node->children) {
        ...
        if (tag == "state" || ends_with(tag, ":state") || tag == "parallel" || ends_with(tag, ":parallel") ||
            ...
            if (!current_parent_state.empty()) {
                ...
                if (parent != nullptr) {
                    ...
                    if (parent->kind == StateKind::Parallel) {
                        // Register parallel child as an orthogonal region root
            ...
            // Set up initial sub-state and requirement traceability links
            ...
            if (curr_state != nullptr) {
                ...
                // Final states in composite parents emit completion events on entry
                if (kind == StateKind::Final && !current_parent_state.empty()) {
                    std::string done_evt = "done.state." + current_parent_state;
                    if (model.find_signal(done_evt) == nullptr) {
                        model.signals.emplace_back(done_evt);
                    }
                    ActionSignature done_act("emit_" + sanitize_identifier(done_evt), "emit(\"" + done_evt + "\")");
                    SignalEmitOp emit_op;
                    emit_op.signal_name = done_evt;
                    done_act.instructions.emplace_back(std::move(emit_op));
                    model.add_action(done_act.name);
                    curr_state->entry_actions.push_back(std::move(done_act));
                }
            }
        }
    }
    ...
    // -------------------------------------------------------------------------
    // Pass 2: Child Element Processing
    // Process nested data models, actions, transitions, and sub-states
    // -------------------------------------------------------------------------
    for (const auto& child : parent_node->children) {
        const std::string tag = child->tag;

        // 1. Data model declarations (<datamodel><data id="..." expr="..."/></datamodel>)
        if (tag == "datamodel" || ends_with(tag, ":datamodel")) {
                ...
                if (data_node->tag == "data" || ends_with(data_node->tag, ":data")) {
                    ...
                    if (!var_id.empty()) {
                        ...
                        if (is_port == "true" || !dir_str.empty()) {
                            ...
                            // Internal state variable declaration
            ...
            // 2. Entry actions (<onentry><assign .../><send .../></onentry>)
            ...
            // 3. Exit actions (<onexit><assign .../><send .../></onexit>)
            ...
            // 4. Hierarchical recursive descent into nested sub-states
            ...
            // 5. History pseudostate declarations (<history type="shallow|deep" id="..."/>)
            ...
            // 7. Long-running activities (<invoke src="..."/>)
            ...
            // 8. Transitions (<transition event="..." cond="..." target="..."/>)
...
void ScxmlParser::parse_scxml_transition(const std::shared_ptr<XmlNode>& trans_node, FsmIr& model,
