...
std::unique_ptr<IParser> create_parser(const std::string& input_file,
...
int main(int argc, char* argv[]) {
  ...
  if (!opts.export_runtime_dir.empty()) {
    ...
    if (!RuntimeExporter::export_runtime(opts.export_runtime_dir,
                                         opts.cpp_standard, export_err)) {
  ...
  const std::string generated_code =
      CppGenerator::generate_header(model, gen_opts);
  ...
  if (!opts.output_file.empty()) {
    ...
    const std::string std_label =
        (opts.cpp_standard == CppStandard::Cpp20) ? "C++20" : "C++17";
    std::cout << "[SUCCESS] " << std_label << " ("
