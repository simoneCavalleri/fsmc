...
ValidationResult FsmValidator::validate(const FsmIr& model) {
    ...
    if (!SemanticAnalyzer::validate(model, sem_errors, sem_warnings)) {
...
void FsmValidator::validate_initial_state(const FsmIr& model, const std::set<std::string>& node_names,
                                          ValidationResult& result) {
...
void FsmValidator::validate_transition_endpoints(const FsmIr& model, const std::set<std::string>& node_names,
                                                 ValidationResult& result) {
...
void FsmValidator::validate_choice_pseudostates(const FsmIr& model, ValidationResult& result) {
    for (const auto& choice_item : model.choice_nodes) {
...
void FsmValidator::validate_reachability(const FsmIr& model, ValidationResult& result) {
...
void FsmValidator::validate_livelock_cycles(const FsmIr& model, ValidationResult& result) {
...
void FsmValidator::validate_deadlock_states(const FsmIr& model, ValidationResult& result) {
...
void FsmValidator::validate_transition_determinism(const FsmIr& model, ValidationResult& result) {
    std::map<std::pair<std::string, std::string>, std::vector<const TransitionEdge*>> trans_by_state_event;
...
void FsmValidator::validate_timed_transitions(const FsmIr& model, ValidationResult& result) {
