...
bool WcetAnalysisPass::run(FsmIr& ir, DiagnosticEngine& diag) {
    ...
    if (!has_zeno_cycle_) {
        ...
        if (max_micro_steps_ > threshold_) {
            diag.report(Diagnostic::warning("W_WCET_HIGH_MICROSTEPS",
                                            "Longest chained micro-step sequence (" + std::to_string(max_micro_steps_) +
                                                ") exceeds configured real-time latency threshold (" +
