/**
 * @file test_multiformat_presets_roundtrip.cpp
 * @brief Formal Verification and End-to-End Roundtrip Suite for Canonical Multi-Format Presets.
 *
 * Test Intent:
 *   Verify that all 6 canonical multi-format models
 *   (SysML v2, SCXML, PlantUML, JSON, Cameo XMI, Mermaid) parse into valid FsmIr,
...
class MultiformatPresets : public ::testing::Test {
  protected:
    fs::path temp_dir;

    void SetUp() override {
        temp_dir = fs::temp_directory_path() / ("fsmc_multiformat_test_" + std::to_string(std::rand()));
    ...
    void run_full_pipeline(const std::string& name, const std::string& source, const std::string& format) {
        ...
        // Middle-End Validation
        auto validation = FsmValidator::validate(model);
        EXPECT_TRUE(validation.is_valid) << "[" << name << "] Model validation failed with errors";

        // Interval analysis (must not crash or deadlock)
        ...
        // 2. Export and Roundtrip across all diagram formats
        std::vector<std::string> target_formats = {"plantuml", "mermaid", "dot"};
        for (const auto& fmt : target_formats) {
            std::string exported = EmitterFactory::emit_diagram(model, fmt);
            EXPECT_FALSE(exported.empty()) << "[" << name << "] Export to " << fmt << " produced empty string";

            // Re-import exported model
            ...
            if (reimport_parser) {
                ...
                EXPECT_TRUE(reimport_ok) << "[" << name << "] Re-parsing " << fmt << " export failed: " << reimport_err;
                if (reimport_ok) {
        ...
        //    Skip if the model uses features not yet supported by the C++ runtime
        //    (e.g. do_activity — ECPP006, change triggers — ECPP007).
        //    Parsing and roundtrip are the primary assertions of this test.
        DiagnosticEngine backend_diag;
        if (!CppBackendValidator::validate_model(model, backend_diag)) {
            GTEST_LOG_(INFO) << "[" << name << "] Skipping C++ generation: model uses unsupported features\n"
                             << backend_diag.render_to_string();
            return;
        }
...
/**
 * @brief Verify Autonomous UAV Mission SysML v2 preset end-to-end.
 * @scenario Ingest OMG SysML v2 UAV Mission model, validate, roundtrip across formats, and compile generated C++
 * headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, Sysml2UavMission_PassesRoundtripAndStrictCompilation) {
    run_full_pipeline("uav_mission", PRESET_SYSML2_UAV, "sysml2");
}

/**
 * @brief Verify Industrial Press SCXML preset end-to-end.
 * @scenario Ingest W3C SCXML Industrial Press model, validate, roundtrip across formats, and compile generated C++
 * headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, ScxmlIndustrialPress_PassesRoundtripAndStrictCompilation) {
    run_full_pipeline("industrial_press", PRESET_SCXML_PRESS, "scxml");
}

/**
 * @brief Verify Connection Manager PlantUML preset end-to-end.
 * @scenario Ingest PlantUML Connection Manager model, validate, roundtrip across formats, and compile generated C++
 * headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, PlantUmlConnectionManager_PassesRoundtripAndStrictCompilation) {
    run_full_pipeline("connection_manager", PRESET_PUML_CONNECTION, "plantuml");
}

/**
 * @brief Verify Smart Thermostat XState JSON preset end-to-end.
 * @scenario Ingest XState JSON Smart Thermostat model, validate, roundtrip across formats, and compile generated C++
 * headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, JsonSmartThermostat_PassesRoundtripAndStrictCompilation) {
    run_full_pipeline("smart_thermostat", PRESET_JSON_THERMOSTAT, "json");
}

/**
 * @brief Verify Satellite Mission Cameo XMI preset end-to-end.
 * @scenario Ingest Cameo MagicDraw XMI Satellite Mission model, validate, roundtrip across formats, and compile
 * generated C++ headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, CameoXmiSatelliteMission_PassesRoundtripAndStrictCompilation) {
    run_full_pipeline("satellite_mission", PRESET_CAMEO_SATELLITE, "cameo");
}

/**
 * @brief Verify Async Motor Controller Mermaid preset end-to-end.
 * @scenario Ingest Mermaid Motor Controller model, validate, roundtrip across formats, and compile generated C++
 * headers.
 * @expected Parsing succeeds, validation passes, diagram roundtrip succeeds, and C++17/C++20 standalone headers compile
 * cleanly.
 */
TEST_F(MultiformatPresets, MermaidAsyncMotorController_PassesRoundtripAndStrictCompilation) {
