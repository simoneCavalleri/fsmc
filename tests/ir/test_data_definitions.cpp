/**
 * @file test_data_definitions.cpp
 * @brief Unit tests for IR user-defined data structures: EnumDefinition, StructDefinition, and JSON schema
 * serialization.
 */
...
/**
 * @brief Verify EnumDefinition and EnumLiteral creation, query methods, and value semantics.
 * @scenario EnumLiteral with default/explicit values, custom underlying integer type, and mutation operations.
 * @expected Enum lookup correctly identifies members, updates literals, and preserves equality semantics.
 */
TEST(EnumDefinition, LiteralInstantiationAndQueryMethods_PreservesValueSemantics) {
...
/**
 * @brief Verify StructDefinition and StructField attributes, ISQ units, domain contracts, and equality.
 * @scenario StructField with type, default initializer, ISQ physical units, and min/max interval bounds.
 * @expected StructDefinition accurately registers fields, allows mutation, and respects equality checks.
 */
TEST(StructDefinition, FieldAttributesAndDomainContracts_PreservesValueSemantics) {
...
/**
 * @brief Verify FsmIr metamodel container integration, lookup methods, and canonical sorting.
 * @scenario Enums and structs registered in non-alphabetical order inside FsmIr model.
 * @expected Query lookups locate definitions and canonicalize() sorts entities deterministically.
 */
TEST(FsmIr, UserDefinedTypesIntegration_PreservesCanonicalOrderingAndLookup) {
    FsmIr ir;
...
/**
 * @brief Verify lossless JSON IR serialization for user-defined enums and struct definitions.
 * @scenario Build FsmIr with enum and struct definitions including contracts, units, and descriptions.
 * @expected JSON representation contains full schema definitions matching original metadata.
 */
TEST(FsmIrSerializer, DataDefinitionsJsonSerialization_PreservesRoundtripFidelity) {
    FsmIr ir;
    ...
    EXPECT_NE(json.find("\"type\": \"float32\""), std::string::npos);
...
/**
 * @brief Verify diagram JSON emission of enums and structs for JSON schema export.
 * @scenario Model with 'EngineState' enum and 'EngineTelemetry' struct serialized to JSON.
 * @expected JSON output formats enum literals and struct fields in compliant diagram representation.
 */
TEST(FsmIrSerializer, CustomEnumsAndStructsDiagramExport_EmitsJsonSchemaSections) {
    FsmIr model;
    ...
    std::string json = FsmIrSerializer::serialize_json(model);
