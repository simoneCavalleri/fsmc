...
void test_cpp17_codegen_generation() {
  ...
  assert(code.find(
         ...
         std::string::npos);
  assert(code.find("using NetworkFSMTable = fsm::transition_table<") !=
         std::string::npos);
...
void test_cpp20_codegen_generation() {
  ...
  assert(code.find("namespace net {") != std::string::npos);
  assert(code.find("[[nodiscard]] constexpr bool operator()(const auto&") !=
         std::string::npos);
  assert(code.find("constexpr void operator()(const auto&") !=
         std::string::npos);
  assert(code.find("using NetworkFSMTable = fsm::transition_table<") !=
         std::string::npos);
