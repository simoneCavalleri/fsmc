...
void print_help(const char* prog_name) {
    std::cout
        ...
        << "       " << prog_name << " -i <model_file> --export <mermaid|plantuml|sysml2|json> -o <out_file>\n"
        ...
        << "  -e, --export <fmt>        Export diagram representation to: 'mermaid', 'plantuml', 'sysml2', 'json'\n"
        ...
        << "Examples:\n"
        << "  " << prog_name << " -i connection.mmd -o connection_fsm.hpp --target cpp --std 20 --name ConnectionFSM\n"
        << "  " << prog_name << " -i spacecraft.sysml -o spacecraft_fsm.hpp --standalone\n"
        << "  " << prog_name << " -i model.xmi --export mermaid -o model.mmd\n"
        << "  " << prog_name << " -i model.sysml --verify\n"
...
CliOptions parse_cli_args(int argc, char* argv[]) {
    ...
    for (int idx = 1; idx < argc; ++idx) {
        ...
        } else if (!arg.starts_with("-") && opts.input_file.empty()) {
            opts.input_file = arg;
        } else {
            opts.is_valid = false;
    ...
    if (opts.target_lang != "cpp" && opts.target_lang != "c++") {
        opts.is_valid = false;
        opts.error_message = "Target backend '" + opts.target_lang +
                             "' is planned for upcoming releases (v1.1+). Currently available backend: 'cpp'.";
        return opts;
    }
...
int main(int argc, char* argv[]) {
    ...
    if (opts.show_version) {
        std::cout << "fsmc v1.0.0 (The Finite State Machine Compiler Infrastructure)\n";
        return 0;
    }
    if (!opts.is_valid) {
    ...
    if (!opts.export_diagram_format.empty()) {
        ...
        } else if (opts.export_diagram_format == "dot" || opts.export_diagram_format == "gv") {
            exported_diagram = DotSerializer::serialize(model);
        } else if (opts.export_diagram_format == "scxml") {
            exported_diagram = ScxmlSerializer::serialize(model);
        } else if (opts.export_diagram_format == "cameo" || opts.export_diagram_format == "xmi") {
            exported_diagram = CameoSerializer::serialize(model);
        } else {
            std::cerr << "Error: Unsupported export diagram format: '" << opts.export_diagram_format
                      << "'. Supported: mermaid, plantuml, sysml2, json, dot, scxml, cameo\n";
            return 1;
        }
